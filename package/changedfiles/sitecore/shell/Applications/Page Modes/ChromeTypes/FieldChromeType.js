Sitecore.PageModes.ChromeTypes.Field = Sitecore.PageModes.ChromeTypes.ChromeType.extend({
        ...
        if (persistedValue) {
            ...
            if (fieldValueInput) {
                ...
                if (this.fieldType === "multi-line text") {
                    if (this.fieldValue.attr("value") !== undefined) {
                        this.fieldValue.attr("value", this.fieldValue.attr("value").replace(/\r?\n/g, "<br\>"))
                    }
                }
    ...
    processPastedContent: function (e) {
        ...
        var fieldType = Sitecore.PageModes.InlineEditingUtil.getClosestFieldNode(e.target).getAttribute("scfieldtype");
        var lineBreak;
        if (fieldType === "multi-line text") {
            lineBreak = 'br';
        }

        var processedContent = this.surrondConentWithLineBreaks(clipboardData.getData("text/plain"), lineBreak);
        if (!processedContent) {
            //e.preventDefault();
