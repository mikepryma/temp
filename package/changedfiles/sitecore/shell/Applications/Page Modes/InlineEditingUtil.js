...
Sitecore.PageModes.InlineEditingUtil = new function () {
    ...
    this.onKeyPress = function (e) {
        ...
        try {
            fieldType = Sitecore.PageModes.InlineEditingUtil.getClosestFieldNode(selection.baseNode).getAttribute("scfieldtype");
        } catch (err) {
            fieldType = selection.baseNode.parentElement.getAttribute("scfieldtype");
        }
        ...
        if ((e.returnValue || Sitecore.PageModes.InlineEditingUtil.isPrintableKey(e))) {
            if (Sitecore.PageModes.InlineEditingUtil.checkEmptyEditableElement(e.target)) {
                Sitecore.PageModes.InlineEditingUtil.prepopulateEmptyElement(e.target, fieldType);
                return;
            }
        }
    ...
    this.isPrintableKey = function (keyPressEventArgs) {
        if (typeof keyPressEventArgs.which == "undefined") {
            ...
            return !keyPressEventArgs.ctrlKey &&
                !keyPressEventArgs.metaKey &&
                !keyPressEventArgs.altKey &&
                keyPressEventArgs.which != 8;
        }
