define(["sitecore", "/-/speak/v1/ExperienceEditor/ExperienceEditor.js", "/-/speak/v1/ExperienceEditor/DOMHelper.js"],
    function(Sitecore, ExperienceEditor, DOMHelper) {
        ...
        var insertPagePageCode = Sitecore.Definitions.App.extend({
            ...
            initialized: function() {
                ...
                this.ItemTreeView.attributes.contentLanguage =
                    ExperienceEditor.RibbonApp.getApp().currentContext.language;
                ...
                $("div[class='dialog-content']").bind("selectOptionPressed",
                    function(event, element) {
                        ...
                        insertPagePageCode.InsertOptions.set("selectedDisplayName",
                            element.attributes["data-sc-displayname"].value);
            ...
            initializeTree: function(context) {
                this.runAction({
                    onActivate: function(node) {
                        ...
                        document.querySelectorAll('[data-sc-id="ItemNameText"]')[0].value = "";
                        context.InsertButton.viewModel.disable();

                        if (document.querySelectorAll('div[data-sc-id="InsertOptions"]')[0].children.length === 0) {
                            document.querySelectorAll('[data-sc-id="ItemNameText"]')[0].value = "";
                            context.InsertButton.viewModel.disable();
                            $('input[data-sc-id="ItemNameText"]').prop("readonly", true);
                        } else {
                            $('input[data-sc-id="ItemNameText"]').prop("readonly", false);
                        }

                        var eeContext = ExperienceEditor.RibbonApp.getAppContext();
                        eeContext.currentContext.itemId = node.data.key;
                        var canInsert = true;
                        ExperienceEditor.Web.postServerRequest("ExperienceEditor.Insert.CanInsert",
                            eeContext.currentContext,
                            function(response) {
            ...
            initializeStructure: function() {
                ...
                ExperienceEditor.PipelinesUtil.generateRequestProcessor("ExperienceEditor.Breadcrumb.GetStructure",
                    function(response) {
            ...
            enableInsertButton: function() {
                ...
                if (context.templateId && context.itemNameElement().value.trim()) {
                    if (document.querySelectorAll('div[data-sc-id="InsertOptions"]')[0].children.length > 0) {
            ...
            setInsertButtonClick: function() {
                this.on("button:insert",
                    function() {
                        var newName = $('input[data-sc-id="ItemNameText"]').val();
                        ...
                        ExperienceEditor.PipelinesUtil.generateRequestProcessor(
                            "ExperienceEditor.Insert.ValidateNewName",
                            function(response) {
                                response.context.app.closeDialog(response.context.templateItemId +
                                    ',' +
                                    response.context.value +
                                    ',' +
                                    response.context.parentItemId);
                            },
